...
the owner moved the radial menu off the touchpad (it's on its button).
...
`[[steam_button]]` entry: touchpad right half → F8 →
`CLICK WowKeysPage:LeftButton` (left half → F7 → previous). `[[steam_button]]` = one controller input
